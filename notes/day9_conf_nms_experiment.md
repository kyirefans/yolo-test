...
| 实验 | conf | nms_iou | predictions | GT | TP | FP | FN | Precision | Recall | Avg IoU |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| D | 0.25 | 0.50 | 5395 | 18581 | 4472 | 923 | 14109 | 0.8289 | 0.2407 | 0.8130 |
| A | 0.25 | 0.70 | 5732 | 18581 | 4591 | 1141 | 13990 | 0.8009 | 0.2471 | 0.8098 |
| E | 0.25 | 0.90 | 9746 | 18581 | 4923 | 4823 | 13658 | 0.5051 | 0.2649 | 0.7948 |
| B | 0.50 | 0.70 | 3452 | 18581 | 3320 | 132 | 15261 | 0.9618 | 0.1787 | 0.8453 |
| C | 0.75 | 0.70 | 1820 | 18581 | 1797 | 23 | 16784 | 0.9874 | 0.0967 | 0.8781 |
...
固定 `nms_iou=0.70`，观察 confidence threshold：

| conf | predictions | TP | FP | FN | Precision | Recall |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0.25 | 5732 | 4591 | 1141 | 13990 | 0.8009 | 0.2471 |
| 0.50 | 3452 | 3320 | 132 | 15261 | 0.9618 | 0.1787 |
| 0.75 | 1820 | 1797 | 23 | 16784 | 0.9874 | 0.0967 |

结论：

```text
confidence threshold 提高后，预测框数量显著减少。
FP 从 1141 降到 132，再降到 23，因此 Precision 明显上升。
但 TP 也从 4591 降到 3320，再降到 1797，FN 不断增加，因此 Recall 明显下降。
```
...
```text
提高 conf 会让模型更保守。
它通常能减少误检、提高 Precision，
但也会过滤掉一部分真实目标对应的预测框，导致漏检增加、Recall 下降。
```

固定 `conf=0.25`，观察 NMS IoU threshold：

| nms_iou | predictions | TP | FP | FN | Precision | Recall |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0.50 | 5395 | 4472 | 923 | 14109 | 0.8289 | 0.2407 |
| 0.70 | 5732 | 4591 | 1141 | 13990 | 0.8009 | 0.2471 |
| 0.90 | 9746 | 4923 | 4823 | 13658 | 0.5051 | 0.2649 |

结论：

```text
NMS IoU threshold 提高后，NMS 更宽松，保留的重叠框更多。
当 iou 从 0.70 提高到 0.90 时，predictions 从 5732 增加到 9746，
FP 从 1141 暴增到 4823，Precision 从 0.8009 下降到 0.5051。
```

同时，Recall 从 `0.2471` 小幅上升到 `0.2649`：

```text
更宽松的 NMS 保留了更多候选框，其中一部分确实匹配到了 GT，
但代价是大量额外 FP。
```

因此，`nms_iou=0.90` 虽然 Recall 略高，但误检代价过大，不适合作为当前默认设置。
...
如果目标是尽量减少误检，可以使用更高 confidence threshold，例如：
...
这个结果说明它找回了稍多目标，但引入了大量误检。

最终结论：

> 在 MOT17 密集行人场景中，YOLO11n 的 Precision 可以通过提高 confidence threshold 变得很高，但 Recall 整体偏低，说明模型对密集、小目标和遮挡行人存在明显漏检。NMS IoU 过高会保留大量重复框并显著降低 Precision；NMS IoU 过低会更严格地压掉重复框，但也可能轻微降低 Recall。
